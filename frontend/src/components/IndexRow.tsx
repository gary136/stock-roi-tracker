...
const IndexRow: React.FC<Props> = ({ ticker, name }) => {
          ...
          {data && (
            <>
              ...
              <span className={data.current.change >= 0 ? 'text-green-600 text-sm' : 'text-red-500 text-sm'}>
                ...
                {Math.abs(data.current.change).toFixed(2)} ({data.current.change_pct.toFixed(2)}%)
              </span>
              <span className={`text-xs font-medium ${zoneColor(data.volume.z_score)}`}>
                Vol {fmtZ(data.volume.z_score)}
              </span>
              {data.volume.obv_direction !== null && (
                <span className={data.volume.obv_direction === 'up' ? 'text-green-600 text-sm' : 'text-red-500 text-sm'}>
                  OBV {data.volume.obv_direction === 'up' ? '↑' : '↓'}
                </span>
              )}
            </>
          )}
          ...
          {!loading && !error && (
            <div className="relative">
              ...
              {data && !data.candles.some(c => c.volume > 0) && (
                <div className="absolute bottom-1 left-2 text-xs text-gray-400 pointer-events-none">
                  Volume N/A
                </div>
              )}
            </div>
